...
const features = [
  ...
  },
  {
    ...
    description: "From IT Parks and hospitals to educational institutions and temples, our diverse project portfolio speaks for our expertise."
  }
];
...
export default function WhyChooseUsSection() {
  ...
  return (
    <section ref={containerRef} className="relative w-full bg-[#fcfcfc] pt-32 pb-16 md:pt-[14rem] md:pb-12 overflow-hidden font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {features.map((feature, index) => {
            ...
            return (
              <div 
                key={index}
                className={`feature-card relative ${isDark ? 'bg-[#2c2c2e] text-white' : 'bg-gray-200 text-[#1c1c1e] shadow-sm'} p-8 md:p-10 rounded-[2rem] hover:-translate-y-2 transition-transform duration-300 flex flex-col min-h-[320px]`}
              >
                <h3 className="text-xl md:text-2xl font-semibold mb-4 leading-tight">
                  {feature.title}
                </h3>
                <p className={`${isDark ? 'text-gray-400' : 'text-gray-500'} text-sm leading-relaxed mb-12`}>
                  {feature.description}
                </p>
                
                {/* Circular Arrow Icon */}
                <div className={`absolute bottom-8 right-8 w-12 h-12 rounded-full flex items-center justify-center transition-transform hover:scale-110 cursor-pointer ${isDark ? 'bg-white text-black' : 'bg-[#1c1c1e] text-white'}`}>
